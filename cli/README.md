...
After installing CodePush CLI globally, it will be available under `dota-standalone`.
...
```
dota-standalone register <optional: server-url>
```
...
```shell
dota-standalone login <optional: server-url>
```
...
```shell
dota-standalone session ls
dota-standalone session rm <machineName>
```
...
```shell
dota-standalone access-key add "VSTS Integration"
```
...
```shell
dota-standalone login --accessKey <accessKey>
```
...
```shell
dota-standalone access-key patch <accessKeyName> --name "new name" --ttl 10d
```
...
```
dota-standalone app add <appName>
```
...
```
dota-standalone app add MyApp-Android
dota-standalone app add MyApp-iOS
```
...
```
dota-standalone app rename <appName> <newAppName>
```
...
```
dota-standalone app rm <appName>
```
...
```shell
dota-standalone collaborator add <appName> <collaboratorEmail>
```
...
```shell
dota-standalone collaborator rm <appName> <collaboratorEmail>
```
...
```shell
dota-standalone collaborator ls <appName>
```
...
```shell
dota-standalone app transfer <appName> <newOwnerEmail>
```

_NOTE: Just like with the `dota-standalone collaborator add` command, this expects that the new owner has already registered with CodePush using the specified e-mail address._

Once confirmed, the specified developer becomes the app's owner and immediately receives the permissions associated with that role. Besides the transfer of ownership, nothing else about the app is modified (e.g. deployments, release history, collaborators). This means that you will still be a collaborator of the app, and therefore, if you want to remove yourself, you simply need to run the `dota-standalone collaborator rm` command after successfully transferring ownership.
...
```
dota-standalone deployment add <appName> <deploymentName>
```

If you want to re-use an existing deployment key, you can do this with:

```
dota-standalone deployment add <appName> <deploymentName> -k <existing-deployment-key>
```

Just like with apps, you can remove and rename deployments as well, using the following commands respectively:

```
dota-standalone deployment rm <appName> <deploymentName>
dota-standalone deployment rename <appName> <deploymentName> <newDeploymentName>
```
...
```
dota-standalone deployment ls <appName> [--displayKeys|-k]
```
...
2. [React Native](#releasing-updates-react-native) - Performs the same functionality as the general release command, but also handles the task of generating the updated app contents for you (JS bundle and assets), instead of requiring you to run both `react-native bundle` and then `dota-standalone release`.
...
```
dota-standalone release <appName> <updateContents> <targetBinaryVersion>
...
This specifies the name of the CodePush app that this update is being released for. This value corresponds to the friendly name that you specified when originally calling `dota-standalone app add` (e.g. "MyApp-Android"). If you need to look it up, you can run the `dota-standalone app ls` command to see your list of apps.
...
```shell
dota-standalone release-react <appName> <platform>
...
dota-standalone release MyApp-iOS ./CodePush 1.0.0
```
...
```shell
dota-standalone release-react MyApp-iOS ios
```
...
```shell
dota-standalone release-react MyApp-Android android -p "./foo/bar/"
dota-standalone release-react MyApp-Android android -p "./foo/bar/build.gradle"
```
...
```shell
dota-standalone release-react MyApp-iOS ios -p "./foo/bar/MyFile.plist"
```
...
dota-standalone release-react MyApp-iOS ios --pre "STAGING"

# Tell the CLI to use your dev plist (`DEV-Info.plist`).
# Note that the hyphen separator can be explicitly stated.
dota-standalone release-react MyApp-iOS ios --pre "DEV-"
```
...
```shell
dota-standalone patch <appName> <deploymentName>
[--label <releaseLabel>]
...
dota-standalone patch MyApp-iOS Production -m

# Increase the rollout for v23 to 50%
dota-standalone patch MyApp-iOS Production -l v23 -rollout 50%
```

### Label parameter

Indicates which release (e.g. `v23`) you want to update within the specified deployment. If ommitted, the requested changes will be applied to the latest release in the specified deployment. In order to look up the label for the release you want to update, you can run the `dota-standalone deployment history` command and refer to the `Label` column.
...
dota-standalone patch MyApp-iOS Staging -t "1.0.0 - 1.0.5"
```
...
```
dota-standalone promote <appName> <sourceDeploymentName> <destDeploymentName>
[--description <description>]
[--label <label>]
[--disabled <disabled>]
[--mandatory]
...
dota-standalone promote MyApp-iOS Staging Production -t "*"
```
...
```
dota-standalone rollback <appName> <deploymentName>
dota-standalone rollback MyApp-iOS Production
```
...
```
dota-standalone rollback MyApp-iOS Production --targetRelease v34
```
...
```
dota-standalone deployment history <appName> <deploymentName>
```
...
```
dota-standalone deployment clear <appName> <deploymentName>
```
...
```shell
dota-standalone release-react <appName> <platform> --privateKeyPath private.pem
```
